...
fun HomeScreen(viewModel: MainViewModel, onPairNewDevice: () -> Unit) {
    ...
    val exercising by viewModel.exerciseActive.collectAsState()
    ...
    ScalingLazyColumn(
        ...
        when (connectionState) {
            is ConnectionState.Connected -> {
                ...
                item {
                    ListHeader {
                        Text("Health Sync", fontSize = 12.sp)
                    }
                }
                item {
                    HealthMetricsRow(
                ...
                item {
                    if (exercising) {
                        Button(
                            ...
                            Text("Stop Workout", fontSize = 12.sp, color = Color.White)
                        }
                    } else {
                        Button(
...
private fun HealthMetricsRow(
    ...
    Column(
        ...
        val ox = spo2?.let { "$it%" } ?: "—"
        val st = steps?.let { "$it" } ?: "—"
        val cal = calories?.let { "$it kcal" } ?: "—"
        val dist = distanceMeters?.let { "${(it / 1000.0).format(1)} km" } ?: "—"
        val sleep = sleepMinutes?.let { "${it / 60}h${it % 60}m" } ?: "—"

        Text("♥ $hr   🫁 $ox", fontSize = 13.sp, color = Color(0xFFE5E7EB))
        Spacer(modifier = Modifier.height(2.dp))
        Text("👟 $st   🔥 $cal   📏 $dist", fontSize = 12.sp, color = Color(0xFF9CA3AF))
        Spacer(modifier = Modifier.height(2.dp))
        Text("😴 $sleep", fontSize = 12.sp, color = Color(0xFF9CA3AF))
    }
}
